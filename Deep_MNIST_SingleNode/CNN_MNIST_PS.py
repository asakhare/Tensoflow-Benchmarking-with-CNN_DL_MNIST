...
nputprint("Input Batch Size=" + str(batch_size))
print("Execution Type: Synchronous")
...
def train():
	with tf.Graph().as_default(),tf.device('/cpu:0'):
			...
			for i in range(0,gpu_num):
					with tf.device(('/gpu:{0}').format(i)):
						with tf.name_scope(('scope_gpu_{0}').format(i)) as infer_scope:
							...
							#list_of_logits.append(yy)
							#list_of_batches.append(y_dict[('y{0}').format(i)])
							correct_predictions.append(tf.equal(tf.argmax(yy, 1), tf.argmax(y_dict[('y{0}').format(i)],1)))#y_dict['y0'], 1))
		...
		#correct_prediction = tf.equal(tf.argmax(list_of_logits, 1), tf.argmax(list_of_batches,1))#y_dict['y0'], 1))
